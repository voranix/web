...
const ALTOKE_API_KEY = defineSecret("ALTOKE_API_KEY");         // altk_... de https://altoke-one.vercel.app/account/api-keys
...
// ---------------------------------------------------------------------
// Integración con Altoke (https://altoke-one.vercel.app), el tracker de
// incidencias que usa el equipo: conecta el Tablero de Tareas del admin
// para poder mandar una tarea como incidencia sin salir del sitio. La
// llave de API es sensible (da acceso de escritura al tracker del
// equipo), así que TODO esto corre server-side — nunca se expone al
// cliente, tal como pide la documentación de Altoke.
// ---------------------------------------------------------------------

const ALTOKE_BASE_URL = "https://altoke-one.vercel.app";

async function altokeRequest(path, options = {}) {
    const apiKey = ALTOKE_API_KEY.value();
    if (!apiKey) throw new HttpsError("failed-precondition", "Falta configurar ALTOKE_API_KEY en el backend.");
    const res = await fetch(`${ALTOKE_BASE_URL}${path}`, {
        ...options,
        headers: {
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
        logger.error(`altokeRequest: ${path} respondió ${res.status}`, data);
        const mensaje = data.error || `Altoke respondió con un error (${res.status}).`;
        throw new HttpsError(res.status === 401 || res.status === 403 ? "permission-denied" : "internal", mensaje);
    }
    return data;
}
...
async function requireStaff(uid) {
    ...
    if (!profile || !profileRoles(profile).some(r => ["admin", "editor"].includes(r))) {
        throw new HttpsError("permission-denied", "Solo staff puede usar la integración con Altoke.");
    }
}

// Los dos de listado son el paso "elegí a qué proyecto/tipo de incidencia
// de Altoke van las tareas de este tablero" — se usan una sola vez desde
// el modal de configuración, después queda guardado en configuracion/altoke.
exports.altokeListarProyectos = onCall({ secrets: [ALTOKE_API_KEY] }, async (request) => {
    await requireStaff(request.auth?.uid);
    const data = await altokeRequest("/api/v1/projects");
    return { projects: data.projects || data || [] };
});

exports.altokeListarTiposIncidencia = onCall({ secrets: [ALTOKE_API_KEY] }, async (request) => {
    await requireStaff(request.auth?.uid);
    const projectId = String(request.data?.projectId || "");
    if (!projectId) throw new HttpsError("invalid-argument", "Falta projectId.");
    const data = await altokeRequest(`/api/v1/projects/${encodeURIComponent(projectId)}/issue-types`);
    return { issueTypes: data.issueTypes || data || [] };
});

function altokeBodyDesdeTarea(tarea) {
    const body = {
        title: tarea.titulo || "Sin título",
        priority: tarea.prioridad || "MEDIUM"
    };
    if (tarea.descripcion) body.description = tarea.descripcion;
    // fechaLimite se guarda como "YYYY-MM-DD" (input type=date); a medianoche
    // UTC alcanza, Altoke solo usa la fecha.
    if (tarea.fechaLimite) body.dueDate = new Date(`${tarea.fechaLimite}T00:00:00Z`).toISOString();
    return body;
}

// Crea la incidencia en Altoke a partir de una tarea existente del tablero
// y guarda la referencia (id + key, ej. "PROJ-14") en la propia tarea para
// no duplicarla ni perder el vínculo.
exports.altokeCrearIncidenciaDesdeTarea = onCall({ secrets: [ALTOKE_API_KEY] }, async (request) => {
    await requireStaff(request.auth?.uid);
    const tareaId = String(request.data?.tareaId || "");
    if (!tareaId) throw new HttpsError("invalid-argument", "Falta tareaId.");

    const db = admin.firestore();
    const [tareaSnap, configSnap] = await Promise.all([
        db.doc(`tareas/${tareaId}`).get(),
        db.doc("configuracion/altoke").get()
    ]);
    if (!tareaSnap.exists) throw new HttpsError("not-found", "La tarea ya no existe.");
    const tarea = tareaSnap.data();
    if (tarea.altoke?.id) throw new HttpsError("already-exists", "Esta tarea ya tiene una incidencia de Altoke vinculada.");

    const config = configSnap.exists ? configSnap.data() : null;
    if (!config?.projectId || !config?.issueTypeId) {
        throw new HttpsError("failed-precondition", "Todavía no se eligió el proyecto de Altoke para este tablero (⚙ Conectar Altoke).");
    }

    const body = { issueTypeId: config.issueTypeId, ...altokeBodyDesdeTarea(tarea) };
    const data = await altokeRequest(`/api/v1/projects/${encodeURIComponent(config.projectId)}/issues`, {
        method: "POST",
        body: JSON.stringify(body)
    });
    const issue = data.issue || data;
    await tareaSnap.ref.update({
        altoke: { id: issue.id, key: issue.key || "", creadoEl: admin.firestore.FieldValue.serverTimestamp() }
    });
    return { id: issue.id, key: issue.key || "" };
});

// Reenvía título/descripción/prioridad/fecha a la incidencia ya vinculada
// cuando se edita la tarea desde el tablero — para no tener el tracker
// desactualizado apenas alguien cambia algo acá.
exports.altokeActualizarIncidenciaDesdeTarea = onCall({ secrets: [ALTOKE_API_KEY] }, async (request) => {
    await requireStaff(request.auth?.uid);
    const tareaId = String(request.data?.tareaId || "");
    if (!tareaId) throw new HttpsError("invalid-argument", "Falta tareaId.");

    const tareaSnap = await admin.firestore().doc(`tareas/${tareaId}`).get();
    if (!tareaSnap.exists) throw new HttpsError("not-found", "La tarea ya no existe.");
    const tarea = tareaSnap.data();
    if (!tarea.altoke?.id) throw new HttpsError("failed-precondition", "Esta tarea no tiene una incidencia de Altoke vinculada.");

    await altokeRequest(`/api/v1/issues/${encodeURIComponent(tarea.altoke.id)}`, {
        method: "PATCH",
        body: JSON.stringify(altokeBodyDesdeTarea(tarea))
    });
    return { ok: true };
});
